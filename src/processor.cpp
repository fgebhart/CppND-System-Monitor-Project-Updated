 ...
 *
 * @param which_cpu: could be e.g. 'cpu' for the aggregate of all cpus or 'cpu0', 'cpu1', ... for individual cpus.
  ...
  if (filestream.is_open()) {
    while (std::getline(filestream, line)) {
      if (line.find(which_cpu) != std::string::npos) {
        ...
        break;  // ensure to only collect the data of the first hit for given 'which_cpu'
      }
    }
  }
  return cpu_data;
...
float Processor::Utilization() {
    ...
    // algorithm for computing the cpu utilization, taken from https://stackoverflow.com/a/23376195/5384702
